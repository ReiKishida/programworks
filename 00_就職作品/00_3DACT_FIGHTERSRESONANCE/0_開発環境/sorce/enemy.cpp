...
// 読み込むテキストファイル
//=====================================
#define MODEL_FILENAME_00 ("data/TEXT/motion_enemy_00.txt")			// スピード型のテキスト
...
char			   CEnemy::m_acFileName[MAX_ENEMY_PARTS][256] = {};		// 読み込むファイル名
LPD3DXMESH		   CEnemy::m_apMesh[MAX_ENEMY_PARTS] = {};				// メッシュ情報へのポインタ
LPD3DXBUFFER	   CEnemy::m_apBuffMat[MAX_ENEMY_PARTS] = {};			// マテリアル情報へのポインタ
DWORD			   CEnemy::m_aNumMat[MAX_ENEMY_PARTS] = {};				// マテリアルの情報の数
CMeshOrbit		  *CEnemy::m_pMeshOrbit = NULL;							// 軌跡のポインタ
//========================================================================================================
// 関数名：コンストラクタ
...
HRESULT CEnemy::Load(void)
{
	...
	// テクスチャの生成
	D3DXCreateTextureFromFile(pDevice, "data/TEXTURE/character/enemy_00.png", &m_pTexture);
...
// 関数の概要：敵シーンを生成する
//=============================================================================
CEnemy *CEnemy::Create(D3DXVECTOR3 pos, D3DXVECTOR3 rot, float fLife, ENEMYTYPE type)
{
	CEnemy  *pEnemy;
...
// 関数の概要：--
//=============================================================================
void CEnemy::EnemySetMove_Interia(void)
{
	//移動量に代入
	m_pos.x += m_move.x;
	m_pos.y += m_move.y;
...
void CEnemy::EnemyMotionState(bool *bAction)
{
	...
	else if (bAction[ENEMYBOOLTYPE_FLOATDAMAGE])
	{
		...
		{//3フレームのとき
			m_move.x += (sinf(m_rot.y - D3DX_PI) * 1.0f) * -10.0f;
			m_move.y += 33.0f;
			m_move.z += (cosf(m_rot.y - D3DX_PI) * 1.0f) * -10.0f;
		}
...
//=============================================================================
void CEnemy::LoadEnemyText(void)
{
	FILE *pFile = fopen(MODEL_FILENAME_00, "r");	// 読み込むテキストファイルのアドレス
...
//=============================================================================
void CEnemy::CollisionObjectEnemy(void)
{
	//オブジェクトのポインタ
	CObject *pObjectTop;
...
// 関数の概要：敵と壁との当たり判定
//=============================================================================
void CEnemy::CollisionWallEnemy(void)
{
	//メッシュウォールのポインタ
...
//=============================================================================
void CEnemy::CollisionCylinderEnemy(void)
{
...
// 関数の概要：--
//=============================================================================
void CEnemy::CollisionAttack(void)
{
...
void CEnemy::CollisionPlayer(void)
{
	...
	while (pPlayer != NULL)//NULLになるまで
	{
		...
		{
			pPlayer->CollisionPlayer(&m_pos, &m_posold, &m_move, &m_rot, 20.0f);
	...
	while (pEnemy != NULL)//NULLになるまで
	{
		...
		if (pEnemy->GetObjType() == CScene::OBJTYPE_ENEMY && this !=pEnemy)
		{
...
// 関数の概要：0になったらコンボ終了
//=============================================================================
void CEnemy::UpdateCancelCombo(void)
{
	//コンボゲージのポインタ
...
// 関数の概要：プレイヤーが半径に入ってきたらtrueを返す
//=============================================================================
bool CEnemy::AttackPlayer(D3DXVECTOR3 * pos, float fRange)
{
	bool bfind = false;
...
// 関数の概要：半径外へ戻す
//=============================================================================
bool CEnemy::CollisionEnemy(D3DXVECTOR3 *pPos, D3DXVECTOR3 *pPosOld, D3DXVECTOR3 *pMove, D3DXVECTOR3 *pRot, float fRadius)
{
	bool bCol = false;
...
// 関数の概要：--
//=============================================================================
void CEnemy::EnemyDebugLog(void)
{
	CDebugProc::Print("\n");
...
void CEnemy::UpdateFind(void)
{
	...
	if (AttackPlayer(&playerpos, 6.5f) == true)
	{
		if (m_state == ENEMYSTATE_NORMAL)
		{
			m_move.x = 0.0f;
...
// 関数の概要：プレイヤーのライフの確認
//=============================================================================
void CEnemy::EnemyLifeCheck(void)
{
