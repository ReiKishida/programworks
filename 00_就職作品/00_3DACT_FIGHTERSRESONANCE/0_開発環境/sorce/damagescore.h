...
public:
	...
	CDamageScore(int nPriority = 7, OBJTYPE type = OBJTYPE_SCORE);	//コンストラクタ
	~CDamageScore();	//デストラクタ
	static CDamageScore *Create(D3DXVECTOR3 pos);	//数字の生成
	HRESULT Init();	//初期化処理
...
private:
	static CNumber *m_apNumber[DAMAGE_SCORE_NUMBER];	//数字
