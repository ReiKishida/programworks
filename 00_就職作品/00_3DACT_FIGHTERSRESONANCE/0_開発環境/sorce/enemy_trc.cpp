...
// 読み込むテキストファイル
//=====================================
#define MODEL_FILENAME_02 ("data/TEXT/motion_enemy_02.txt")			// トリッキーのテキスト


//**************************************************************************************************
// 静的メンバ変数
//**************************************************************************************************
LPDIRECT3DTEXTURE9 CEnemyTrick::m_pTexture = NULL;							// 共有テクスチャのポインタ
char			   CEnemyTrick::m_acFileName[MAX_ENEMY_PARTS][256] = {};		// 読み込むファイル名
LPD3DXMESH		   CEnemyTrick::m_apMesh[MAX_ENEMY_PARTS] = {};				// メッシュ情報へのポインタ
LPD3DXBUFFER	   CEnemyTrick::m_apBuffMat[MAX_ENEMY_PARTS] = {};			// マテリアル情報へのポインタ
DWORD			   CEnemyTrick::m_aNumMat[MAX_ENEMY_PARTS] = {};				// マテリアルの情報の数
CMeshOrbit		  *CEnemyTrick::m_pMeshOrbit = NULL;							// 軌跡のポインタ
//========================================================================================================
// 関数名：コンストラクタ
// 関数の概要：生成されたときに呼び出される。値のクリア
//========================================================================================================
CEnemyTrick::CEnemyTrick(int nPriority, OBJTYPE type) : CScene(nPriority, type)
{
	m_bUseDraw = true;
}
...
HRESULT CEnemyTrick::Load(void)
{
	...
	// テクスチャの生成
	D3DXCreateTextureFromFile(pDevice, "data/TEXTURE/character/enemy_3.png", &m_pTexture);



	FILE *pFile = fopen(MODEL_FILENAME_02, "r");	// 読み込むテキストファイルのアドレス
...
void CEnemyTrick::Unload(void)
{
	...
	//マテリアルの数の破棄
	for (int nCntModel = 0; nCntModel < MAX_ENEMY_PARTS; nCntModel++)
	{
...
// 関数の概要：敵シーンを生成する
//=============================================================================
CEnemyTrick *CEnemyTrick::Create(D3DXVECTOR3 pos, D3DXVECTOR3 rot, float fLife, ENEMYTYPE type)
{
	CEnemyTrick  *pEnemyTrc;
	pEnemyTrc = new CEnemyTrick;//シーンクラスの生成
	if (pEnemyTrc != NULL)
	{
		pEnemyTrc->m_pos = pos;
		//pEnemyTrc->m_rot.y = -D3DX_PI * 0.5f;
		pEnemyTrc->m_rot = rot;
		pEnemyTrc->m_fDestAngle = pEnemyTrc->m_rot.y;
		pEnemyTrc->m_fLife = fLife;
		pEnemyTrc->m_type = type;
		pEnemyTrc->m_posold = pEnemyTrc->m_pos;
		pEnemyTrc->m_bAction[ENEMYBOOLTYPE_JUMP] = false;
		pEnemyTrc->Init();
	}
	return pEnemyTrc;
}
...
HRESULT CEnemyTrick::Init(void)
{
	...
	// 2Dシーンのオブジェクトの設定
	SetObjType(CScene::OBJTYPE_ENEMY_TRC);
...
// 関数の概要：敵の影を描画する
//=============================================================================
void CEnemyTrick::DrawShadow(void)
{
	CRenderer *pRenderer;
	pRenderer = CManager::GetRenderer();
...
// 関数の概要：敵のパーツを描画する
//=============================================================================
void CEnemyTrick::DrawParts(void)
{
	CRenderer *pRenderer;
	pRenderer = CManager::GetRenderer();
...
// 関数の概要：敵の移動関係
//=============================================================================
void CEnemyTrick::EnemyMove(void)
{
...
// 関数の概要：攻撃系の処理
//=============================================================================
void CEnemyTrick::EnemyAction(void)
{
...
// 関数の概要：落下と地面への生成
//=============================================================================
void CEnemyTrick::EnemyFallen(void)
{
	//落下
...
// 関数の概要：ブレンド設定や、モーション
//=============================================================================
void CEnemyTrick::EnemyMotion(void)
{
	//サウンドの取得
	CSound *pSound;
	pSound = CManager::GetSound();
...
// 関数の概要：敵の状態変化
//=============================================================================
void CEnemyTrick::EnemyState(void)
{
	switch (m_state)
	{
...
// 関数の概要：--
//=============================================================================
void CEnemyTrick::EnemySetMove_Interia(void)
{
	//移動量に代入
	m_pos.x += m_move.x;
	m_pos.y += m_move.y;
...
// 関数の概要：敵の状態遷移
//=============================================================================
void CEnemyTrick::EnemyMotionState(bool *bAction)
{
	//カメラの取得
	CCamera *pCamera;
	...
	{// 敵が弱攻撃をしたなら
		...
		if (m_nCounterAllFrame == 13 && m_Motionstate == MOTION_LIGHTATTACK)
		{//13フレームかつ中パンチモーションのとき
			ModelMtx = m_apModel[6]->GetMatrix();
			//判定の発生
			CCollision::CreateAttack(D3DXVECTOR3(ModelMtx._41, ModelMtx._42, ModelMtx._43), D3DXVECTOR3(0.0f, 0.0f, 0.0f), 2, 25.0f, CCollision::COLLSIONTYPE_ENEMY_LIGHTATTACK, m_pos);
		}
	}
	...
	{// 敵が強攻撃をしたなら
		...
		if (m_nCounterAllFrame == 30 && m_Motionstate == MOTION_HEAVYATTACK)
		{//13フレームかつ中パンチモーションのとき
...
//=============================================================================
void CEnemyTrick::EnemyRotFixes(D3DXVECTOR3 rot)
{
	// X軸の修正
	if (rot.x > D3DX_PI)
	{// +3.14より、超えてしまったら
...
// 関数の概要：大きければ、引く、小さければ、足す
//=============================================================================
void CEnemyTrick::EnemyRotFixes(float rot)
{
	if (rot > D3DX_PI)
	{// +3.14より、超えてしまったら
		rot -= D3DX_PI * 2.0f;
...
//=============================================================================
void CEnemyTrick::LoadEnemyText(void)
{
	FILE *pFile = fopen(MODEL_FILENAME_02, "r");	// 読み込むテキストファイルのアドレス
...
// 関数の概要：--
//=============================================================================
void CEnemyTrick::InitEnemy(void)
{
	//敵情報
...
// 関数の概要：--
//=============================================================================
void CEnemyTrick::InitMotion(void)
{
	m_nNumKey = 2;
...
// 関数の概要：パーツの初期設定を行う
//=============================================================================
void CEnemyTrick::DefaultParts(void)
{
	for (int nCntKey = 0; nCntKey < m_aKeyInfo[m_Motionstate].nNumKey; nCntKey++)
	{//総フレーム数の計算
		m_nAllFrame += m_aKeyInfo[m_Motionstate].aKey[nCntKey].nFrame;
...
//=============================================================================
void CEnemyTrick::CollisionObjectEnemy(void)
{
	//オブジェクトのポインタ
	CObject *pObjectTop;
...
// 関数の概要：敵と壁との当たり判定
//=============================================================================
void CEnemyTrick::CollisionWallEnemy(void)
{
	//メッシュウォールのポインタ
...
//=============================================================================
void CEnemyTrick::CollisionCylinderEnemy(void)
{
...
// 関数の概要：--
//=============================================================================
void CEnemyTrick::CollisionAttack(void)
{
	//当たり判定のポインタ
...
// 関数の概要：半径外へ戻す
//=============================================================================
void CEnemyTrick::CollisionPlayer(void)
{
	//敵のポインタ
	CPlayer *pPlayer;
	...
	while (pEnemyTrc != NULL)//NULLになるまで
	{
		...
		if (pEnemyTrc->GetObjType() == CScene::OBJTYPE_ENEMY_TRC && this != pEnemyTrc)
		{
			pEnemyTrc->CollisionEnemy(&m_pos, &m_posold, &m_move, &m_rot, 40.0f);
	...
	while (pEnemyPow != NULL)//NULLになるまで
	{
		...
		if (pEnemyPow->GetObjType() == CScene::OBJTYPE_ENEMY_POW)
		{
			pEnemyPow->CollisionEnemy(&m_pos, &m_posold, &m_move, &m_rot, 40.0f);
...
// 関数の概要：体力ゲージの生成
//=============================================================================
void CEnemyTrick::CreateGauge(void)
{
...
// 関数の概要：体力ゲージの位置を代入する
//=============================================================================
void CEnemyTrick::SetGauge(void)
{
	for (int nCntGauge = 0; nCntGauge < 3; nCntGauge++)
	{
		if (m_apLifeGauge[nCntGauge] != NULL)
...
// 関数の概要：0になったらコンボ終了
//=============================================================================
void CEnemyTrick::UpdateCancelCombo(void)
{
	//コンボゲージのポインタ
	CComboGauge *pComboGauge;
	pComboGauge = CGame::GetComboGauge(1);
...
//=============================================================================
bool CEnemyTrick::FindPlayer(D3DXVECTOR3 *pos, float fRange)
{
	bool bfind = false;
...
// 関数の概要：プレイヤーが半径に入ってきたらtrueを返す
//=============================================================================
bool CEnemyTrick::AttackPlayer(D3DXVECTOR3 * pos, float fRange)
{
	bool bfind = false;
...
// 関数の概要：半径外へ戻す
//=============================================================================
bool CEnemyTrick::CollisionEnemy(D3DXVECTOR3 *pPos, D3DXVECTOR3 *pPosOld, D3DXVECTOR3 *pMove, D3DXVECTOR3 *pRot, float fRadius)
{
	bool bCol = false;
...
// 関数の概要：--
//=============================================================================
void CEnemyTrick::EnemyDebugLog(void)
{
	CDebugProc::Print("\n");
...
// 関数の概要：--
//=============================================================================
void CEnemyTrick::UpdateFind(void)
{
	//プレイヤーの取得
	CPlayer *pPlayer;
...
// 関数の概要：敵の位置情報を返す
//=============================================================================
D3DXVECTOR3 CEnemyTrick::GetPos(void)
{
	return m_pos;
}
...
// 関数の概要：敵の角度情報を返す
//=============================================================================
D3DXVECTOR3 CEnemyTrick::GetRot(void)
{
	return m_rot;
}
...
// 関数の概要：敵の移動量を返す
//=============================================================================
D3DXVECTOR3 CEnemyTrick::GetMove(void)
{
	return m_move;
}
...
// 関数の概要：プレイヤーのライフの確認
//=============================================================================
void CEnemyTrick::EnemyLifeCheck(void)
{
