...
public:
	...
	typedef enum
	{//敵の種類
		ENEMYTYPE_SPEED = 0,	// スピード型
		ENEMYTYPE_HEAVY,		// 重量型
		ENEMYTYPE_TRICKY,		// トリッキー型
	...
	{// フラグの種類
		...
		HITTYPE_SKILL,				//プレイヤーの攻撃(強キック)
		HITTYPE_MAX
	}HITTYPE;

	CEnemy(int nPriority = 1, OBJTYPE type = OBJTYPE_ENEMY);	// コンストラクタ
	~CEnemy();													// デストラクタ
	static HRESULT Load(void);									// テクスチャの読み込み
	static void Unload(void);									// テクスチャの破棄
	static CEnemy *Create(D3DXVECTOR3 pos, D3DXVECTOR3 rot, float fLife, ENEMYTYPE type);		// 敵の生成
...
private:
	...
	int m_aParent[MAX_ENEMY_PARTS];						// パーツの親
	int m_aIndex[MAX_ENEMY_PARTS];						// パーツの番号

														//敵の基本情報//
														//静的メンバ変数//
	static LPDIRECT3DTEXTURE9 m_pTexture; //共有テクスチャのポインタ
	static char						 m_acFileName[MAX_ENEMY_PARTS][256];	// 読み込む
	static LPD3DXMESH				 m_apMesh[MAX_ENEMY_PARTS];				// メッシュ情報へのポインタ
	static LPD3DXBUFFER				 m_apBuffMat[MAX_ENEMY_PARTS];			// マテリアル情報へのポインタ
	static DWORD					 m_aNumMat[MAX_ENEMY_PARTS];			// マテリアルの情報の数
