...
LPDIRECTINPUTDEVICE8 g_pDevKeyboard = NULL;	//入力デバイスへのポインタ(キーボード)
BYTE g_aKeyState[NUM_KEY_MAX];		//キー入力情報(プレス情報)
BYTE g_aKeyStateTrigger[NUM_KEY_MAX];	//キーボード入力情報(トリガー情報)
BYTE g_aKeyStateRelease[NUM_KEY_MAX];	//キーボード入力情報(リリース)
...
{
	// 入力デバイス(キーボード)の開放
	if (g_pDevKeyboard != NULL)	//NULLチェック
	{
