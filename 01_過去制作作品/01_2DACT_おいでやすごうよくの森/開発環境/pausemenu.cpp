...
void InitPauseMenu(void)
{
	...
	g_nNowSelect = CONTINUE;
		...
		g_aPauseMenu[CONTINUE].pos = D3DXVECTOR3(SCREEN_WIDTH / 2 - 200, 270, 0);
		g_aPauseMenu[RETRY].pos = D3DXVECTOR3(SCREEN_WIDTH / 2 - 200, 370, 0);
		g_aPauseMenu[QUIT].pos = D3DXVECTOR3(SCREEN_WIDTH / 2 - 200, 470, 0);
...
void UpdatePauseMenu(void)
{
	...
	if (g_aPauseMenu[g_nNowSelect].state == PAUSEMENUSTATE_SELECT)
	{
		...
		g_pVtxBuffPauseMenu->Unlock();
			...
			if (GetKeyboardTrigger(DIK_W) == true || GetKeyboardTrigger(DIK_UPARROW) == true)
			{//Wキー
			...
			else if (GetKeyboardTrigger(DIK_S) == true || GetKeyboardTrigger(DIK_DOWNARROW) == true)
			{//Sキー
				PlaySound(SOUND_LABEL_SE_SELECT);
