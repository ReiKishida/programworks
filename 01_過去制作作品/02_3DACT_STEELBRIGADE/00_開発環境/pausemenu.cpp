...
void InitPauseMenu(void)
{
	...
	g_nNowPauseSelect = CONTINUE;
	
		g_aPauseMenu[CONTINUE].pos = D3DXVECTOR3(SCREEN_WIDTH / 2 , 270, 0);
		g_aPauseMenu[RETRY].pos = D3DXVECTOR3(SCREEN_WIDTH / 2 , 370, 0);
		g_aPauseMenu[QUIT].pos = D3DXVECTOR3(SCREEN_WIDTH / 2 , 470, 0);
...
void UpdatePauseMenu(void)
{
	...
	if (g_aPauseMenu[g_nNowPauseSelect].state == PAUSEMENUSTATE_SELECT)
	{
		// 頂点バッファをロックし、頂点情報へのポインタを取得
		g_pVtxBuffPauseMenu->Lock(0, 0, (void**)&pVtx, 0);
		switch (g_aPauseMenu[g_nNowPauseSelect].uState)
		{
		case PAUSEMENUSTATE_NONE:
			g_aPauseMenu[g_nNowPauseSelect].col.a -= 0.02f;
			if (g_aPauseMenu[g_nNowPauseSelect].col.a <= 0.0f)
			{
				g_aPauseMenu[g_nNowPauseSelect].col.a = 0.0f;
				g_aPauseMenu[g_nNowPauseSelect].uState = PAUSEMENUSTATE_USE;
			}
			break;

		case PAUSEMENUSTATE_USE:
			g_aPauseMenu[g_nNowPauseSelect].col.a += 0.02f;
			if (g_aPauseMenu[g_nNowPauseSelect].col.a >= 1.0f)
			{
				g_aPauseMenu[g_nNowPauseSelect].col.a = 1.0f;
				g_aPauseMenu[g_nNowPauseSelect].uState = PAUSEMENUSTATE_NONE;
			}
			break;
		}
		// 頂点バッファをアンロックする
		g_pVtxBuffPauseMenu->Unlock();
			...
			if (GetKeyboardTrigger(DIK_W) == true || GetKeyboardTrigger(DIK_UPARROW) == true || (state.Gamepad.sThumbLY > XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE && pJoyState.bConnectionJoypad == true))
			{//Wキー
				if (g_JoyStatePause.nJoypadState == INPUT_JOYSTATE_NOTPUSH)
				{
					//PlaySound(SOUND_LABEL_SE_SELECT);
					g_nNowPauseSelect = (g_nNowPauseSelect + (MAX_MENU - 1)) % MAX_MENU;//選択状態を一つ上げる
					//PushPauseMenu();
				}
			...
			else if (GetKeyboardTrigger(DIK_S) == true || GetKeyboardTrigger(DIK_DOWNARROW) == true || (state.Gamepad.sThumbLY < -XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE && pJoyState.bConnectionJoypad == true))
			{//Sキー
				if (g_JoyStatePause.nJoypadState == INPUT_JOYSTATE_NOTPUSH)
				{
					//PlaySound(SOUND_LABEL_SE_SELECT);
					g_nNowPauseSelect = (g_nNowPauseSelect + 1) % MAX_MENU;//選択状態を一つ下げる
					//PushPauseMenu();
				}
				g_JoyStatePause.nJoypadState = INPUT_JOYSTATE_PUSH;
			...
			else if (GetKeyboardTrigger(DIK_RETURN) == true || (state.Gamepad.wButtons &XINPUT_GAMEPAD_B && pJoyState.bConnectionJoypad == true))
			{//RETURNキー
				if (g_JoyStatePause.nJoypadState == INPUT_JOYSTATE_NOTPUSH)
				{
					//PushPauseMenu();
					switch (g_nNowPauseSelect)
					{//現在選択中の状態によって処理を分岐
