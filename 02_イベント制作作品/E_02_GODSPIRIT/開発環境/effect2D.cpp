...
{
	{ "data/TEXTURE/Pill.png" },	// 毒
	{ "data/TEXTURE/Sunder001.jpg" },// 電気
...
void InitEffect2D(void)
{
	...
	for (int nCntEffect2D = 0; nCntEffect2D < MAX_EFFECT_2D; nCntEffect2D++)
	{
		...
		g_aEffect2D[nCntEffect2D].fRadius = 0.0f;							// エフェクトの半径
...
void UpdateEffect2D(void)
{
	...
	for (int nCntEffect2D = 0; nCntEffect2D < MAX_EFFECT_2D; nCntEffect2D++, pVtx += 4)
	{
		...
		{// エフェクトが使用されている
			...
			if (g_aEffect2D[nCntEffect2D].fRadius <= 0)
			{// 反転防止
				g_aEffect2D[nCntEffect2D].fRadius = 0;
			...
			{
			case EFFECTTYPE_2D_ARROW:	// ちょうどのエフェクト
				g_aEffect2D[nCntEffect2D].col -= D3DXCOLOR(0.0f, 0.0f, 0.0f, 0.01f);	// 透明度加算
				g_aEffect2D[nCntEffect2D].fRadius -= 0.2f;		// 半径を縮小
				break;
			case EFFECTTYPE_2D_SUNDER:
				g_aEffect2D[nCntEffect2D].col.a -= 0.1f;
				
				break;
			}
			g_aEffect2D[nCntEffect2D].nCounterAnim++;
			if (g_aEffect2D[nCntEffect2D].nType == EFFECTTYPE_2D_SUNDER)
			{
				if (g_aEffect2D[nCntEffect2D].nCounterAnim % 5 == 0)
				{
					g_aEffect2D[nCntEffect2D].nPatternAnim = (g_aEffect2D[nCntEffect2D].nPatternAnim + 1) % 4;
					// テクスチャの設定
					pVtx[0].tex = D3DXVECTOR2((g_aEffect2D[nCntEffect2D].nPatternAnim % 4) *0.25f, (g_aEffect2D[nCntEffect2D].nPatternAnim / 4)*1.0f);
					pVtx[1].tex = D3DXVECTOR2((g_aEffect2D[nCntEffect2D].nPatternAnim % 4)*0.25f + 0.25f, (g_aEffect2D[nCntEffect2D].nPatternAnim / 4)*1.0f);
					pVtx[2].tex = D3DXVECTOR2((g_aEffect2D[nCntEffect2D].nPatternAnim % 4)*0.25f, (g_aEffect2D[nCntEffect2D].nPatternAnim / 4)*1.0f + 1.0f);
					pVtx[3].tex = D3DXVECTOR2((g_aEffect2D[nCntEffect2D].nPatternAnim % 4)*0.25f + 0.25f, (g_aEffect2D[nCntEffect2D].nPatternAnim / 4)*1.0f + 1.0f);
				}
			}
			else
			{
			...
			// 頂点情報の設定
			pVtx[0].pos = D3DXVECTOR3(-(g_aEffect2D[nCntEffect2D].fRadius), -(g_aEffect2D[nCntEffect2D].fRadius), 0.0f) + g_aEffect2D[nCntEffect2D].pos;
			pVtx[1].pos = D3DXVECTOR3((g_aEffect2D[nCntEffect2D].fRadius), -(g_aEffect2D[nCntEffect2D].fRadius), 0.0f) + g_aEffect2D[nCntEffect2D].pos;
			pVtx[2].pos = D3DXVECTOR3(-(g_aEffect2D[nCntEffect2D].fRadius), (g_aEffect2D[nCntEffect2D].fRadius), 0.0f) + g_aEffect2D[nCntEffect2D].pos;
			pVtx[3].pos = D3DXVECTOR3((g_aEffect2D[nCntEffect2D].fRadius), (g_aEffect2D[nCntEffect2D].fRadius), 0.0f) + g_aEffect2D[nCntEffect2D].pos;
...
void SetEffect2D(D3DXVECTOR3 pos,D3DXVECTOR3 move, D3DXCOLOR col, float fRadius, int nLife,EFFECTTYPE_2D nType)
{
	...
	for (int nCntEffect2D = 0; nCntEffect2D < MAX_EFFECT_2D; nCntEffect2D++, pVtx += 4)
	{
		...
		{// 弾がが使用されていない
			...
			g_aEffect2D[nCntEffect2D].fRadius = fRadius;	// 半径
			...
			if (g_aEffect2D[nCntEffect2D].nType == EFFECTTYPE_2D_SUNDER)
			{
				g_aEffect2D[nCntEffect2D].nPatternAnim = rand() % 4;
				// テクスチャの設定
				pVtx[0].tex = D3DXVECTOR2((g_aEffect2D[nCntEffect2D].nPatternAnim % 4) *0.25f, (g_aEffect2D[nCntEffect2D].nPatternAnim / 4)*1.0f);
				pVtx[1].tex = D3DXVECTOR2((g_aEffect2D[nCntEffect2D].nPatternAnim % 4)*0.25f + 0.25f, (g_aEffect2D[nCntEffect2D].nPatternAnim / 4)*1.0f);
				pVtx[2].tex = D3DXVECTOR2((g_aEffect2D[nCntEffect2D].nPatternAnim % 4)*0.25f, (g_aEffect2D[nCntEffect2D].nPatternAnim / 4)*1.0f + 1.0f);
