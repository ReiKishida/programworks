...
#define WINDOW_NAME		"速攻帰宅！！定時君！"	// ウインドウのキャプション名
#define WINDOW_MODE		(false)		// false最大化 trueウインドウ
//******************************************************************************************************************************
// プロトタイプ宣言
//******************************************************************************************************************************
...
HRESULT Init(HINSTANCE hInstance, HWND hWnd, BOOL bWindow)
{
	...
	InitFade(g_mode);
	// プレイヤーモデル読み込み
	SetPlayerFileData(0,"data/TEXT/teiji.txt");
	SetObjectFileData();
	// モードの設定
	SetMode(g_mode);
...
void SetMode(MODE mode)
{
	...
	case MODE_RESULT:	// リザルト
		InitResult();
		PlaySound(SOUND_RESULT_HAC01);
		ChangeVolume(SOUND_RESULT_HAC01, 0.5f);
		PlaySound(SOUND_RESULT_HAC02);
		break;
	case MODE_RANKING:	// スタッフロール
		InitRanking();
